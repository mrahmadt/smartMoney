...
<div class="shadow-lg mx-2 rounded-md border border-gray-300 bg-white px-4 py-2">
    <div class="grid grid-flow-row-dense grid-cols-2 grid-rows-1">
      <div>
        <div class="text-sm font-light text-gray-400 whitespace-nowrap">{{__('cards.Remaining')}}</div>
      ...
      <div>
        <div style="width: 100px; height: 80px; float: right; position: relative;">
            <div class="proportional-nums" style="width: 85%; height: 40px; position: absolute; top: 69%; left: 0; margin-top: -20px; line-height:19px; text-align: center; z-index: 999999999999999">{{$budget_percentage_used}}%</div><canvas id="budgetSpendingChart" width="100" height="100"></canvas>
        </div>
    </div>
    </div>
</div>
...
<script>
    var budgetSpendingChart_config = {
      type: 'doughnut',
      data: {
        datasets: [{
